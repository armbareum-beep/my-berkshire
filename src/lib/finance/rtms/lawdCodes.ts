/**
 * 전국 시군구 법정동코드(앞 5자리) — RTMS LAWD_CD 파라미터용 정적 상수.
 *
 * 출처: 행정표준코드관리시스템 법정동코드 전체자료(시군구 레벨, 말소·출장소 제외)
 * + 2023-05 이후 개편 반영: 강원특별자치도(42→51), 전북특별자치도(45→52),
 * 군위군 대구 편입(47720→27720), 부천시 구 재설치(41192/41194/41196), 세종(36110).
 * 구(區)가 있는 시는 구 단위 코드만 수록 — RTMS 는 자치구/일반구 레벨로 조회한다.
 * 행정구역 개편(연 0~2회) 시 이 파일만 갱신하면 된다.
 */

export interface LawdCode {
  /** 법정동코드 앞 5자리 (RTMS LAWD_CD). */
  code: string;
  /** 시/도명. */
  sido: string;
  /** 시/군/구명 (일반구는 "성남시 분당구" 형태). */
  sigungu: string;
}
...
export const LAWD_CODES: LawdCode[] = [
  ...
  { code: "41192", sido: "경기도", sigungu: "부천시 원미구" },
  { code: "41194", sido: "경기도", sigungu: "부천시 소사구" },
  { code: "41196", sido: "경기도", sigungu: "부천시 오정구" },
  ...
  // 세종특별자치시 (1)
  { code: "36110", sido: "세종특별자치시", sigungu: "세종시" },
  // 강원특별자치도 (18)
  ...
  { code: "52145", sido: "전북특별자치도", sigungu: "익산시함열출" },
