...
/**
 * 같은 시점의 주식수 fact 들을 합산(값 기준 dedupe — 여러 제출본 재보고 중복 제거).
 * 단일 클래스는 정확. 복수 클래스(버크셔 A/B 등)는 재무제표가 A주 환산 단일값으로만
 * 보고하는 경우가 많아 여기서 클래스 단위를 완전히 정합시키진 못한다 — 그 잔여 왜곡은
 * lookThrough 의 "지분 단위 정합성 가드"(내재 PER<1이면 레그 제외)가 최종 방어한다.
 */
