...
export type ProductKind = "stock" | "etf" | "deposit" | "bond";
/** exposure: [한국, 중국, 미국, 기타 주식, 채권]. bondCountry splits the bond part by 한국·중국·미국·기타. */
export type Product = { name: string; price: number | null; exposure: number[]; source?: string; kind?: ProductKind; bondCountry?: number[] };
...
export function validatePortfolio(input: unknown): Portfolio {
  ...
  check(Object.values(p.products).every(v=>(v.kind===undefined||["stock","etf","deposit","bond"].includes(v.kind))&&(v.bondCountry===undefined||shares(v.bondCountry,4))), "상품 유형과 채권 국가 비중(100%)을 확인해 주세요.");
  check(p.accounts.every(a=>a.cashByCurrency===undefined||(a.cashByCurrency&&typeof a.cashByCurrency==="object"&&!Array.isArray(a.cashByCurrency)&&Object.entries(a.cashByCurrency).every(([c,v])=>/^[A-Z]{3}$/.test(c)&&num(v))&&Math.abs(Object.values(a.cashByCurrency).reduce((x,y)=>x+y,0)-a.cash)<=1)), "통화별 현금 합계가 계좌 현금과 같은지 확인해 주세요.");
