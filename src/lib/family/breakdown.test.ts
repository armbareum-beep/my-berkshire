...
function sample(): Portfolio {
  ...
  p.accounts = [account("general", "위탁종합", 1000), account("cma", "CMA(RP형)", 500), account("isa", "개인종합자산관리계좌", 0), account("pension", "연금저축", 0), account("irp", "퇴직연금운용", 0), account("yen", "외화예금", 0)];
  ...
  p.products = {
    ...
    "JPYKRW=X": { name: "엔화", price: 9, exposure: [0, 0, 0, 0, 1], kind: "cash", currency: "JPY" },
  ...
  p.holdings = [
    ...
    { account: "yen", code: "JPYKRW=X", quantity: 100, cost: 950 },
  ];
  return p;
}
...
describe("계좌 유형 필터", () => {
  it("계좌 유형을 기본·ISA·IRP·연금저축·CMA로 나눔", () => {
    ...
    expect(accountGroup("외화예금")).toBe("fx");
  });
  it("선택한 유형의 계좌만 집계", () => {
    const p = sample();
    ...
    expect(summarize(p, "all", "all", "all", "fx").nav).toBe(900);
    expect(summarize(p, "all", "all", "all", "all").selected).toHaveLength(6);
  });
});
...
describe("보유상품 합산", () => {
  ...
  it("수익금·수익률 높은순·낮은순, 미평가는 항상 마지막", () => {
    ...
    expect(sortHoldings(rows, "profit", true).map(r => r.code)).toEqual(["005930", "283580", "FD", "284430", "JPYKRW=X", "META", "NOPRICE"]);
    expect(sortHoldings(rows, "rate", false).map(r => r.code)).toEqual(["META", "JPYKRW=X", "284430", "FD", "283580", "005930", "NOPRICE"]);
...
describe("유형별 국가 비중", () => {
  ...
  it("ETF·개별주식·채권은 국가별, 현금은 통화별로 나누고 합계가 총자산과 같음", () => {
    ...
    expect(b.cash).toEqual([["KRW", 1400], ["USD", 70], ["JPY", 30 + 900]]);
  ...
  it("목표 비교의 기타에는 현금·채권이 들어감", () => {
    ...
    expect(actual[3]).toBe(1500 + 500 + 2000 + 900);
  });
  it("통화별 현금 합계와 채권 국가 비중을 검증", () => {
    ...
    expect(() => validatePortfolio(r)).toThrow(/통화/);
  });
});
