...
export function performanceComparison(p: Portfolio, owner = "all", account = "all", broker = "all") {
  ...
  const unavailable = (reason: string) => ({ rate: null as number | null, exIncomeRate: null as number | null, incomeContribution: null as number | null, reason, history });
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    if (prior <= 0 || adjusted < 0 || exIncomeAdjusted < 0) return unavailable("잔고가 0이거나 입출금 시점의 영향이 커서 이 기간의 TWR을 확정할 수 없어요.");
    growth *= adjusted / prior;
    exIncomeGrowth *= exIncomeAdjusted / prior;
    if (!Number.isFinite(growth) || !Number.isFinite(exIncomeGrowth)) return unavailable("이 기간의 운용성과를 계산할 수 없어요.");
  }
  const rate = growth - 1, exIncomeRate = history.income ? exIncomeGrowth - 1 : null;
  return { rate, exIncomeRate, incomeContribution: exIncomeRate === null ? null : rate - exIncomeRate, reason: "", history };
}
