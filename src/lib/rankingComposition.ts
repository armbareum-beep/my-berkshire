/**
 * 랭킹 프로필 시트 구성 비중 — 유형별(주식/ETF/원자재/코인/현금) 비중 %만 담는 jsonb(v1).
 * 정확한 금액·수량·종목명은 계산 도중에만 쓰고 절대 저장하지 않는다(비공개 불변식 — 034 정합,
 * 035에서 XIRR·자산 구간으로 공개 범위가 완화됐어도 정확 금액·종목명은 계속 구조적 비공개).
 */
...
/**
 * 종목별 보유수량×현재가 + 현금을 유형별로 합산해 비중 %만 반환.
 * 시세 실패(priceAvailable=false)면 null(금액을 알 수 없는데 %를 추정해 보여줄 수 없음).
 * 표시 순서는 ASSET_TYPE_ORDER(주식→ETF→원자재→코인) + 현금 마지막, 보유한 유형만.
...
export function computeCompositionPct(params: {
  ...
  const { positions, prices, cash, meta, priceAvailable } = params;
