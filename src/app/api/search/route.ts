...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 로컬 카탈로그 결과를 앞에 두고 야후 결과 병합(코드 기준 중복 제거).
    const seen = new Set<string>(local.map((l) => l.symbol));
    const results: SymbolSearchResult[] = [...local];
    ...
    // ETF TER 병렬 조회: 미국=Yahoo quoteSummary(crumb), 한국=KRX
    ...
    const [usTermap, krTermap] = await Promise.all([
      ...
      krEtfSymbols.length > 0
        ? fetchKrxEtfTers(krEtfSymbols, supabase)
