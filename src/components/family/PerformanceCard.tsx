...
export default function PerformanceCard({ data, owner, account, broker, group }: { data: Portfolio; owner: string; account: string; broker: string; group: string }) {
  ...
  return <section className="surface performance-card">
    ...
    <p className="performance-caption">같은 기간, 독립 CMA·외화예금을 제외하고 입출금 영향을 조정한 성과를 비교해요.</p>
    ...
    <p className="fine">비교값은 지수를 추종하는 국내 ETF의 원화 총수익률이며 분배금 재투자를 반영합니다. 독립 CMA·외화예금 계좌는 제외하지만 투자계좌 안의 예수금은 포함합니다.</p>
    {h && <details className="method"><summary>비교 기준과 추정 범위</summary><p>{h.note}</p><p>총수익은 각 날짜의 (종료 자산 − 순입금) ÷ 전일 자산을 연결합니다. 배당·이자 제외 수익률은 여기에 실제 세후 입금된 배당과 이자를 추가로 차감합니다. 세전액을 빼지 않으므로 원천징수세를 수익에서 두 번 차감하지 않습니다.</p><p>독립 CMA·외화예금과 투자계좌 사이의 이체는 투자계좌 기준 외부 입출금으로 처리합니다. 거래 시각을 알 수 없어 입출금은 일말로 가정합니다. 배당·이자 제외 수익률은 현재 총수익형 비교 ETF와 직접 비교하지 않습니다.</p><p>이 카드는 {h.end}까지 확인된 거래·일별 가격 기준으로 고정됩니다. ‘시세 갱신’은 자산 현황만 갱신하며 비교 기간을 늘리지 않습니다. 이후 자료를 반영할 때 함께 갱신해요.</p></details>}
  </section>;
}
