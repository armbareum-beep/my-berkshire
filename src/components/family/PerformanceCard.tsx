...
export default function PerformanceCard({ data, owner, account, broker }: { data: Portfolio; owner: string; account: string; broker: string }) {
  ...
  return <section className="surface performance-card">
    ...
    <div className="performance-grid"><div className="performance-own"><span>투자계좌 · 추정 TWR</span><strong className={result.rate !== null && result.rate < 0 ? "negative" : "positive"}>{result.rate === null ? "자료 확인 필요" : percent(result.rate)}</strong><small>원화 · 실제 기간 수익률</small></div>
    ...
    <p className="fine">비교값은 지수를 추종하는 국내 ETF의 원화 수익률이며 분배금 재투자를 반영합니다. 독립 CMA 계좌는 제외하지만 투자계좌 안의 예수금은 포함합니다.</p>
    {h && <details className="method"><summary>비교 기준과 추정 범위</summary><p>{h.note}</p><p>각 날짜의 (종료 자산 − 순입금) ÷ 전일 자산을 연결합니다. 독립 CMA와 투자계좌 사이의 이체는 투자계좌 기준 외부 입출금으로 처리합니다. 거래 시각을 알 수 없어 입출금은 일말로 가정합니다. ETF는 보수·추적오차로 지수 자체의 수익률과 다를 수 있어요.</p><p>이 카드는 {h.end}까지 확인된 거래·일별 가격 기준으로 고정됩니다. ‘시세 갱신’은 자산 현황만 갱신하며 비교 기간을 늘리지 않습니다. 이후 자료를 반영할 때 함께 갱신해요.</p></details>}
  </section>;
}
