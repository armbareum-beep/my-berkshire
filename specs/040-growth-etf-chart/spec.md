...
해결 방향: ETF 포트폴리오 페이지의 도넛 차트(기능·UI 동일)를 마이버크셔 탭의 "내 지분 실적" 카드 **위쪽**에 그대로 재사용해 배치한다. "내 지분 실적" 카드는 `/lookthrough`(내 지분 실적·내 지분 소식 상세)로 이어지는 진입점이므로, 그 위에 배치하면 두 섹션보다 상단에 차트가 위치하게 된다.
...
ETF를 보유한 사용자가 마이버크셔 탭을 열면, "복리 무중단" 카드 아래·"내 지분 실적" 카드 위에 ETF 배분 도넛 차트가 표시된다. 차트는 ETF 포트폴리오 페이지와 동일하게 ETF/섹터/지역/자산 탭 전환, 세그먼트 하이라이트, 3% 미만 "기타" 묶음을 지원한다.

**Independent Test**: ETF 보유 계좌로 마이버크셔 탭 진입 시 도넛 차트가 "내 지분 실적" 카드보다 위에 렌더링되어야 한다.

**Acceptance Scenarios**:

1. **Given** ETF를 1개 이상 보유한 사용자가, **When** 마이버크셔 탭에 진입하면, **Then** "내 지분 실적" 카드 위에 ETF 배분 도넛 차트가 표시된다.
...
ETF가 없는 사용자에게는 차트를 표시하지 않는다. 기존의 ETF 잠금 카드("ETF를 보유하면 열립니다")가 그대로 안내 역할을 유지한다.

**Acceptance Scenarios**:

1. **Given** ETF를 보유하지 않은 사용자가, **When** 마이버크셔 탭에 진입하면, **Then** 차트 영역 없이 기존 레이아웃(내 지분 실적 → ETF 잠금 카드)이 유지된다.
...
- **FR-001**: 마이버크셔 탭은 ETF 보유 시 "내 지분 실적" 카드 위에 ETF 배분 도넛 차트를 표시해야 한다.
...
- `src/app/growth/page.tsx`: 기존 `etfSlices` 계산에 `value`·`etfWeight` 필드와 `totalEtfValue`를 추가하고, `CompoundingStreakCard`와 "내 지분 실적" 블록 사이에 `<Suspense fallback={<ChartSkeleton />}><EtfChartStreamed …/></Suspense>` 삽입 (`hasEtf` 게이트).
- `src/components/etf/ChartSkeleton.tsx`: `/etf-portfolio` 페이지에 있던 로컬 스켈레톤을 공용 컴포넌트로 추출(두 페이지 공유).
