...
FR-001~006 전부 구현됨. 단 ETF `etf_included` 레이블 대신 실제 코드에서는 `included`로 통합.

**PR #24** — `feat(lookthrough): ETF PER 기반 귀속 순이익 투시 합산`
- `src/lib/finance/lookThrough.ts`: ETF 케이스에서 `getEtfStats()`로 PER 조회 → `netIncomeMine = positionValue / per`
- LegStatus: `etf_pending` → `etf_no_per`(PER 없을 때) / `included`(정상 합산)
- 한국 ETF PER 없을 때 `yahooProxy` 심볼로 fallback 조회

**PR #25** — 패시브 지수 ETF(SPY/QQQ 등)는 투시 대상에서 제외 결정
- 이유: 패시브 인덱스 ETF는 시장 그 자체 → 버핏式 귀속 순이익 의미 없음. 개별 액티브 보유 종목만 투시 대상.
- `trackedIndex` 있는 ETF는 투시 화면에 "인덱스 ETF — 투시 제외" 표시

### 주요 파일
- `src/lib/finance/lookThrough.ts` — ETF leg 분기 로직
- `src/lib/finance/etfStats.ts` — Yahoo PER 조회 + yahooProxy fallback
- `src/components/growth/LookThroughCard.tsx` — leg 상태별 UI 표시

### 연관 작업
- `022-etf-country`: ETF `underlyingCountry` 도입 → countryOf() 보정
- `025-etf-allocation-fix`: loadSecurityMeta stale DB 수정(ETF 국가 오분류)
