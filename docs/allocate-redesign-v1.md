...
> ⚠️ **§2·§6 의 화면 구조는 `allocate-rail-v2.md` 로 대체됐다(2026-08-13 같은 날).**
> 이 문서가 만든 "보기 / 실행 / 설정" 3분할은 조각들을 **동시에 늘어놓아** 사용자가 매번
> "이 중 뭘 해야 하지"를 고르게 만들었다. v2 는 같은 조각을 **한 화면의 3단계 레일**로
> 세운다. Phase 1(평면 목표비중)·Phase 4(설정 통합)·Phase 5(레거시 은퇴)는 v2 에서도 유효하다.
