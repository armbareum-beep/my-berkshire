...
| 드로다운 인내 기록(−10% 구간 매도 무 통과) | [drawdown.ts](../src/lib/finance/drawdown.ts) `computeDrawdownEpisodes`, [drawdownEpisodes.ts](../src/lib/drawdownEpisodes.ts), 14일 창 1회 축하 + 연혁 영구 기록 |
| 자본배분 계획 완수 기록 | [plan.ts](../src/lib/plan.ts) `planCompletionDate`, [milestones.ts](../src/lib/finance/milestones.ts) `journeyMilestones` 지난 주년 전부 + 완수 계획 |
| 규율 등급업 축하(분기별 1회) | [styleHistory.ts](../src/lib/styleHistory.ts) `StyleHistorySnapshot` score/gradeLabel, [celebration.ts](../src/lib/celebration.ts) `gradeUp`, `/growth` 방문 시 스냅샷 자동 저장 |
| 복리 무중단 카운터 카드(마이버크셔) | [CompoundingStreakCard.tsx](../src/components/growth/CompoundingStreakCard.tsx), `/report`·`/growth` 수치 연동 |
| 보고서 회장님 호칭(CFO·연차) | [ReportContent.tsx](../src/app/report/), [AnnualReportView.tsx](../src/components/report/) 주주→회장님 전환, 주주 제거 완료 |
...
전환 절차:
1. Vercel 프로젝트 env에 `CRON_SECRET`(신규 랜덤 값 생성 필요)·`SUPABASE_SERVICE_ROLE_KEY`(prod에 이미 있는지 확인, 없으면 추가) 설정.
...
3. 1주 관찰(cron 로그·`kis_security_master.fetched_at` 갱신 확인) 후 PC schtasks의 KIS 마스터 태스크만 해제(KRX·로고 태스크는 유지):
   `schtasks /Delete /TN "ENUF KIS Security Master Sync" /F`
   (태스크명 출처: [registerKisMasterTask.ps1](../scripts/registerKisMasterTask.ps1))

---

## D5 — 033 게이미피케이션 강화 (2026-07-03 완료)

**spec/033-gamification-honest-roman** 5개 사용자 스토리(드로다운 인내·연혁 영구화·등급업 축하·복리 카드·회장님 카피) 완료 + 마이그레이션 프로덕션 적용.

| 항목 | 상태 | 비고 |
|---|---|---|
| 마이그레이션 `holdings.archived_plans` | ✅ | 2026-07-03 `supabase db push` CLI 프로덕션 적용 완료. jsonb default '[]'. XIRR 포함 asset_scope 규칙 유지(부동산 제외). |
| 사용자 스토리 1~5 구현 | ✅ | US1(드로다운 인내): drawdown.ts 엔진 + 14일 축하 |  
|| | US2(연혁 영구화): archived_plans append + journeyMilestones 주년/완수 계획 |
|| | US3(등급업 축하): StyleHistorySnapshot gradeLabel + 분기 1회 축하 |
|| | US4(복리 카드): CompoundingStreakCard + /growth 배치 |
|| | US5(회장님 카피): ReportContent·AnnualReportView 주주→회장님 전환 |
| 전체 게이트(typecheck·lint·test·build) | ✅ | npm run typecheck 249/249, npm test 통과, npm run build 그린 |
