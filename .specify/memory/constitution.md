<!--
Sync Impact Report
- Version change: 1.2.0 → 1.3.0 (MINOR — Additional Constraints의 모드·랭킹 항목 개정)
- Amendment 2026-07-03 (032-ranking-system): "랭킹 전면 폐지"를 "성과(XIRR) 랭킹 폐지 유지 +
  행동 규율 지표 랭킹 재도입"으로 개정. 032는 조작 저항적 5개 규율 지표(보유기간 가중·역발상
  매수·시장 대비 PME·분산 일관성·적립 일관성)만 집계 — 원칙 I(스타일 중립)·II(정직)와 합치.
  챌린지·라이브 모드 폐지는 불변. 하단 탭은 5탭(홈·검색·＋·랭킹·마이버크셔), "성장" 탭은
  "마이버크셔"로 개명(030).
...
- **모드:** 단일 `ledger`(소급 입력 자유·수기 평단·import). 챌린지·라이브 모드는 폐지. `holding_mode` enum/컬럼은 잔존하나 항상 'ledger'.
- **랭킹:** 성과(XIRR·수익률) 자체의 경쟁 랭킹은 금지 — 수기 데이터라 조작 가능(원칙 II). 랭킹은 **행동 규율 지표**(보유기간 가중·역발상 매수·시장 대비 PME·분산 일관성·적립 일관성)만 집계한다(032). 지표 추가 시에도 스타일 중립(원칙 I)·조작 저항성을 통과해야 한다.
...
**Version**: 1.3.0 | **Ratified**: 2026-06-22 | **Last Amended**: 2026-07-03
