...
$logFile = Join-Path $logDirectory "kis-master-sync.log"
$npm = (Get-Command npm.cmd -ErrorAction Stop).Source

New-Item -ItemType Directory -Path $logDirectory -Force | Out-Null
Set-Location $projectRoot

"[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Starting KIS security master sync" | Add-Content $logFile
try {
  & $npm run sync:kis-master *>> $logFile
